...
cd ..とすることで、newdirectoryディレクトリからweek1_01ディレクトリに移動できることが確認できます
...
- チーム開発が地獄 →　「あれ俺の修正が他の人に消された、、？？？」

- 複数人でのコードの共有ができない → 「ソースコードはメールで送らないといけない？？」

Git/GitHubがあれば、こうした問題をかなり防げます。

- 変更履歴が残る → 「いつ・どのファイルを・どう変えたか」がわかるので、バグが出ても原因をたどりやすい
- 前の状態に戻せる → 「やっぱりこの修正だめだった」となっても、以前の安全な状態に戻せる
- 複数人で同時に開発しやすい → 誰がどの変更をしたかが整理されるので、チーム開発がかなり進めやすくなる
- コード共有が簡単になる → メール添付ではなく、GitHub上で最新のソースコードを共有できる
- レビューがしやすい → Pull Request などを使って、「この変更で大丈夫か」を確認しながら進められる
- バックアップにもなる → ローカルPCが壊れても、GitHubに保存されていれば復元しやすい

つまり、Git/GitHubは
「コードの変更を安全に管理し、個人開発もチーム開発も進めやすくする仕組み」
です。
...
このGitHub上のリポジトリに、自分の手元で変更してアップロードするところまでを動画でお見せします。先程まで利用していたweek1_setupというフォルダを使ってVSコードを開いています。

以下のコードを実行するだけではあります

```jsx
git clone https://github.com/kondotaichi/new.git （自分のリポジトリ名に設定してください）
cd new
...
originはリモートのリポジトリ名（プッシュ先のGitHubのリポジトリ）、mainはプッシュ対象のブランチです。

-uをつけることで、これ以降の操作でoriginとmainをセットにしてね！という意味になる

本来、git pushコマンドはgit push （push先のリポジトリ）（push対象のブランチ）という記法で書く必要があるが、push -u origin mainとすると、これ以降セットでoriginとmainが対応することが明示され、これ以降pushするときは git pushと打つだけで可能になる。
...
pushのコマンドはgit push （リポジトリ名）（ブランチ名）になります。
